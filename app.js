    ...
    if(!result.success){
      ...
      if(window.FDB){
        try{
          ...
          if(idx && idx.schoolId){
            await DB.loadFromFirestore(idx.schoolId);
            const subUsers = DB.get('users',[]);
            const subUser = subUsers.find(u=>u.email===email&&u.passwordHash===pwHash);
            if(subUser){
              this.schoolId = idx.schoolId;
              subUser.lastLogin=new Date().toISOString(); DB.set('users',subUsers);
...
<div class="kpi-card"><div class="kpi-icon ${totalFees-totalExp>0?'green':'amber'}">${SMS._kpiSvg('trending')}</div><div class="kpi-val" style="color:${totalFees-totalExp>0?'var(--success)':'var(--danger)'}">${fmt(totalFees-totalExp)}</div><div class="kpi-label">Net Balance</div></div>
  ...
  async saveUser(){
    ...
    DB.set('users',users);
    const _sid=window.SMS&&window.SMS.schoolId;
    if(_sid&&window.FDB){ FDB.batchWrite(_sid,'users',[newUser]).catch(()=>{}); }
    // Create Firebase Auth account via Cloud Function (works on both dev & prod)
    if(_sid&&window.firebase&&firebase.app().functions){
      firebase.app().functions().httpsCallable('createSubUser')({schoolId:_sid,email,password:pwd,name,role,userId:newUser.id}).catch(e=>console.warn('createSubUser:',e?.message));
    }
    this.audit('Add User','create',`New user: ${name} (${role})`); this.toast('User created!','success'); this.closeModal('m-user'); this.renderUsers();
  },

  deleteUser(id){ const users=DB.get('users',[]); const u=users.find(x=>x.id===id); DB.set('users',users.filter(x=>x.id!==id)); const _sid=window.SMS&&window.SMS.schoolId; if(_sid&&window.FDB){ FDB.delete(_sid,'users',id).catch(()=>{}); } if(_sid&&u?.email&&window.firebase&&firebase.app().functions){ firebase.app().functions().httpsCallable('deleteSubUser')({schoolId:_sid,userId:id,email:u.email}).catch(e=>console.warn('deleteSubUser:',e?.message)); } this.audit('Delete User','delete',`Removed user: ${u?.name}`); this.toast('User removed','warn'); this.renderUsers(); },
